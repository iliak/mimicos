...
static UBYTE	_CurrentDrive = 0;

// set when IRQ fires
static volatile UBYTE _FloppyDiskIRQ = 0;
...
void flpydsk_write_dor(UBYTE val)
{
...
void flpydsk_send_command(UBYTE cmd)
{
...
void flpydsk_write_ccr(UBYTE val)
{
...
__inline void flpydsk_wait_irq()
{
...
// configure drive
void flpydsk_drive_data(ULONG stepr, ULONG loadt, ULONG unloadt, BOOL dma)
{
...
int flpydsk_calibrate(UBYTE drive)
{
...
// read a sector
void flpydsk_read_sector_imp(UBYTE head, UBYTE track, UBYTE sector)
{
	ULONG st0, cyl;
...
void flpydsk_set_working_drive(UBYTE drive)
{
	if (drive < 4)
		_CurrentDrive = drive;
}
...
UBYTE flpydsk_get_working_drive()
{
...
// read a sector
UBYTE* flpydsk_read_sector(int sectorLBA)
{
	if (_CurrentDrive >= 4)
		return 0;
...
void flpydsk_init(int irq)
{
	kprintf("TRCKDK", "init()\n");

	// install irq handler
	setvect(irq, i86_flpy_irq, 0);
	...
	TrackdiskBase.Library.IdString = "trackdisk.device";
	TrackdiskBase.Library.Version = 1;
	TrackdiskBase.Library.Revision = 0;
	AddDevice(&TrackdiskBase);
