...
msgFailure db 0x0D, 0x0A, "*** FATAL: Missing or corrupt KERNEL.EXE. Press Any Key to Reboot.", 0x0D, 0x0A, 0x0A, 0x00
...
main:
	...
	mov		eax, 0x0
	mov		ds, ax
	mov		di, 0x1000					; Location of the memory map
	call	BiosGetMemoryMap
	mov		dword [boot_info+multiboot_info.mmap_addr], 0x1000
	mov		dword [boot_info+multiboot_info.mmap_length], ebp 

	call	LoadRoot					; Read the root entry
   	mov    	ebx, 0		
   	mov		ebp, IMAGE_RMODE_BASE		; Physical address of the kernel
   	mov 	esi, ImageName				; Kernel filename
	call	LoadFile					; load the kernel

   	mov   	dword [ImageSize], ecx 		; Store the kernel size
	cmp		ax, 0						; Load failed ?
	je		EnterStage3		
	mov		si, msgFailure				; Message error
	call   	Puts16						; Display it
	mov		ah, 0		
...
EXECUTE:
	...
	mov		ebx, 0
	mov		edx, [ImageSize]			; Number of block used by the kernel
