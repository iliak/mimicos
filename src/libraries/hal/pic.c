...
__inline void pic_send_command(UBYTE cmd, UBYTE picNum)
{

	if (picNum > 1)
		return;

	UBYTE	reg = (picNum == 1) ? I86_PIC2_REG_COMMAND : I86_PIC1_REG_COMMAND;
	outportb(reg, cmd);
}


// send data to PICs
__inline void pic_send_data(UBYTE data, UBYTE picNum)
{

	if (picNum > 1)
		return;

	UBYTE	reg = (picNum == 1) ? I86_PIC2_REG_DATA : I86_PIC1_REG_DATA;
	outportb(reg, data);
}


// read data from PICs
__inline UBYTE pic_read_data(UBYTE picNum)
{
...
// Initialize pic
void pic_initialize(UBYTE base0, UBYTE base1)
{
