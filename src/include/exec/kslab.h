...
// Opaque data structure that defines a slab. Exported only to kmem.h
struct	kslab;
...
// 
struct kslab_cache * slab_cache_create(STRPTR name,
			ULONG object_size,
			ULONG pages_per_slab,
			ULONG min_free_objects,
			ULONG cache_flags);

ret_t	slab_cache_destroy(struct kslab_cache *cache);
...
// 
vir_addr slab_cache_alloc(struct kslab_cache *cache, ULONG flags);
struct kslab_cache * slab_cache_prepare(vir_addr kernel_core_base,
...
struct kslab_cache * slab_setup_prepare(
	vir_addr			kernel_core_base,
	vir_addr			kernel_core_top,
	ULONG				sizeof_struct_range,
	struct kslab **		first_struct_slab_of_caches,
	vir_addr*			first_slab_of_caches_base,
...
BOOL slab_setup_commit(
	struct kslab *			first_struct_slab_of_caches,
	struct kmem_range *		first_range_of_caches,
	struct kslab *			first_struct_slab_of_ranges, 
	struct kmem_range *		first_range_of_ranges);
